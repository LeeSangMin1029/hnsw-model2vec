...
// ---------------------------------------------------------------------------
// CPU fallback (always available, no cubecl dependency)
// ---------------------------------------------------------------------------

/// Pure-CPU batch L2 squared using the SIMD-optimized `distance` module.
pub(crate) fn cpu_batch_l2_squared(
    ...
    (0..n_vectors)
        .map(|i| {
            let start = i * dim;
            let end = start + dim;
            crate::distance::L2Distance.distance(query, &database[start..end])
        })
        .collect()
}
