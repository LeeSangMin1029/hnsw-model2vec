...
use v_hnsw_cli::commands::file_utils::{generate_id, normalize_source};
...
fn process_changes(changed: &[PathBuf], db_path: &Path, input_path: &Path) {
    ...
    let crates = v_code_intel::mir_edges::detect_changed_crates(input_path, changed);
    if crates.is_empty() {
    ...
    let mir_chunks = match v_code_intel::mir_edges::load_all_mir_chunks(&mir_out_dir) {
        Ok(c) => c,
        Err(e) => {
...
fn update_db(
    ...
    let mut engine = v_hnsw_storage::StorageEngine::open_exclusive(db_path)
    ...
    for entry in entries {
        ...
        engine.insert(id, &[0.0], payload, &embed_text)?;
    }
...
fn rebuild_graph_cache(
    ...
    v_code_intel::loader::save_chunks_cache(&cache_path, &chunks);
