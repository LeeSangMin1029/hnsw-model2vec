...
use crate::filters::{FilterChain, TokenFilter};
...
impl KoreanTokenizer {
    /// Create a new Korean tokenizer with default settings.
    pub fn new() -> Result<Self, VhnswError> {
        Self::with_config(KoreanTokenizerConfig::default())
    }

    /// Create a new Korean tokenizer with custom configuration.
    pub fn with_config(config: KoreanTokenizerConfig) -> Result<Self, VhnswError> {
        let mode = match config.mode {
            TokenizerMode::Normal => Mode::Normal,
            TokenizerMode::Search => Mode::Normal, // Lindera doesn't have search mode for ko-dic
        };

        // Load the ko-dic dictionary (embedded)
        let dictionary = load_embedded_dictionary(DictionaryKind::KoDic).map_err(|e| {
            VhnswError::Tokenizer(format!("failed to load ko-dic dictionary: {}", e))
        })?;
    ...
    /// Create a tokenizer with a user dictionary.
    pub fn with_user_dictionary(
        config: KoreanTokenizerConfig,
        _user_dict: &UserDictionary,
    ) -> Result<Self, VhnswError> {
        // Note: Lindera's user dictionary support requires compiling the dictionary.
        // For now, we'll use the standard dictionary.
        // A full implementation would compile the user dictionary.

        // For now, just create without user dictionary
...
mod tests {
    ...
    fn test_korean_tokenizer_creation() {
        let result = KoreanTokenizer::new();
        assert!(result.is_ok());
    }

    #[test]
    fn test_korean_tokenizer_basic() {
        let tokenizer = KoreanTokenizer::new().expect("create tokenizer");
