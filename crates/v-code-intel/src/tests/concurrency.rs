//! Concurrency tests — verifies v-code DB operations are safe under parallel access.
//!
//! Scenarios:
...
/// This test demonstrates the race condition that `locked_edit` (in v-code)
...
/// With file locking (same `.lock` sidecar mechanism as v-code's `locked_edit`),
