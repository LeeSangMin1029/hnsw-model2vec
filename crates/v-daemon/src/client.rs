//! v-daemon client utilities — communicate with the running daemon process.
//!
//! Provides port discovery, RPC communication, and daemon auto-start.
//! Server implementation lives in the `v-daemon-server` crate.
...
pub fn spawn_daemon(db: &Path) {
    ...
    // Try next to current exe first, fallback to PATH
