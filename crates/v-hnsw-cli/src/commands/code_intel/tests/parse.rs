use crate::commands::code_intel::parse::*;
...
fn parse_forward_slash_path_preserved() {
    let text = "[function] run\nFile: crates/v-hnsw-cli/src/main.rs:1-50";
    let chunk = parse_chunk(text).unwrap();
    assert_eq!(chunk.file, "crates/v-hnsw-cli/src/main.rs");
...
fn normalize_path_deep_windows() {
    assert_eq!(
        normalize_path(".\\crates\\v-hnsw-cli\\src\\main.rs"),
        "crates/v-hnsw-cli/src/main.rs"
    );
}
