...
fn make_entry(name: &str, calls: &[&str]) -> CodeChunkEntry {
    CodeChunkEntry {
        chunk: crate::chunk_code::CodeChunk {
            name: name.to_string(),
            kind: crate::chunk_code::CodeNodeKind::Function,
            text: String::new(),
...
fn called_by_index_qualified_calls() {
    ...
    // Bare name extraction: "Module::helper" → "helper"
...
fn called_by_index_deduplicates() {
    ...
    let index = build_called_by_index(&entries);
    let callers = index.get("target").unwrap();
    // Should deduplicate
...
fn lookup_called_by_qualified_name() {
    ...
    // Looking up "MyStruct::new" should find callers via bare name "new"
